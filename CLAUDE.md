...
- Stage mode (projector): `?stage` on the URL, the Stage/Phone button in the top bar (shown ≥900px), or the S key; remembered in localStorage `std-stage`; forces the slate palette. At ≥1000px the Deal becomes three columns (drawing, band, move list with DSCR effect and toggle), the header carries the nav (`.snav`), and the remaining sections sit in two columns below. `?stage` also opens on the Deal. Screenshot it with a wide viewport; `tools/shoot.js` only covers phones.
