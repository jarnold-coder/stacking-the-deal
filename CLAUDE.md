...
- Stage mode (projector): `?stage` on the URL, the Stage/Phone button in the top bar (shown ≥900px), or the S key; remembered in localStorage `std-stage`; forces the slate palette. At ≥1000px the Deal becomes three columns (drawing, band, move list with DSCR effect and toggle), the header carries the nav (`.snav`), content is capped at 2000px wide, the drawing at `(100vh - 340px)` tall, and the capital-stack bars and paycheck waterfall fill the row beneath the band and the list; the proforma and dials follow full-width. `?stage` also opens on the Deal. In stage the Primer is scaled for a projector (about 1.5× type, 1180px measure) and the hidden views stay hidden: stage layout rules must target `#view-deal:not([hidden])`, or the Deal leaks under the Primer. Screenshot it with a wide viewport; `tools/shoot.js` only covers phones.
