...
- Wide screens (≥760px, not stage): the Deal is two columns. `.deal-left` (heading, drawing, facts, both bands) is sticky and scrolls internally if the viewport is short; the drawing is sized from the viewport height (`calc((100dvh - 560px) * .824)`) so the band stays in view on a laptop; moves, math sheets and the remaining sections scroll on the right. On phones `.deal-left` is `display:contents` so the compact strip can stick.
...
- Stage mode (projector): `?stage` on the URL, the Stage/Phone button in the top bar (shown ≥900px), or the S key; remembered in localStorage `std-stage`; forces the slate palette. At ≥1000px the Deal becomes three columns (drawing, band, move list with DSCR effect and toggle), the header carries the nav (`.snav`), and the remaining sections sit in two columns below. `?stage` also opens on the Deal. In stage the Primer is scaled for a projector (about 1.5× type, 1180px measure) and the hidden views stay hidden: stage layout rules must target `#view-deal:not([hidden])`, or the Deal leaks under the Primer. Screenshot it with a wide viewport; `tools/shoot.js` only covers phones.
